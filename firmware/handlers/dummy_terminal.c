...
extern uint16_t glBufSize;

CyBool_t glDummyTermIsActive = CyFalse;
uint32_t glDMARxCount = 0; /* Counter to track number of buffers received. */
uint32_t glDMATxCount = 0; /* Counter to track number of buffers transmitted. */
...
void dummyTermCallback (
  ...
  if (type == CY_U3P_DMA_CB_PROD_EVENT) {
    ...
    glDMARxCount++;
    log_debug("DUMMY FX3 TERM received buffer %d.\n", glDMARxCount);
  } else if (type == CY_U3P_DMA_CB_CONS_EVENT) {
    log_debug(" Consumer event\n");
...
void dummy_term_init_handler(void) {
  ...
  dmaCfg.size  = (glBufSize * CY_FX_EP_BURST_LENGTH);
